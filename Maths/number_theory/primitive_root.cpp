...
inline ll test(ll x, vi &tnt, ll p, ll phi){
	for(auto xd: tnt){
		if(mod_pow(x, phi/xd, p) == 1) return 0;
	}
	return 1;
}
inline ll Pri_Root(ll x){
	// upper bound es log^6(x) usando la hipotesis de rieman
	ll top = log2(x) + 1, phi = x;
	top = sqr(top) * sqr(top) * sqr(top);
	top = min(top, x);
	make(x);
	vii v;
	vi tnt;
	construct(v);
	for(auto xd: v) phi /= xd.ff, phi *= xd.ff - 1;
	make(phi);
	v.clear();
	construct(v);
	for(auto xd: v) tnt.pb(xd.ff);
	FER(i, 2, top + 1) {
		if(test(i, tnt, x, phi)) {
			return i;
		}  
