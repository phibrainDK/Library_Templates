...
inline void debug(T x) {
	trace(x.l, x.r, x.ta, x.is_cut, x.asc, x.id, x.node);
}
struct PQTree {
    ll n, boc;
    long long ans;
    ...
    inline void dfs(ll u, ll pp, ll x) {
        auto calcuate = [&](vi vec) {
            if(sz(vec) <= 2) return 0LL;
            ll p = 1, acum, ph;
            long long resp = 0LL;
            while(p < sz(vec) - 1) {
                acum = 0, ph = p;
                while(ph < sz(vec) - 1 and acum < x) acum += vec[ph ++];
                if(acum < x) break;
                resp += mul(mul(p, (sz(vec) - ph), mod1), (sz(vec) - ph + 1), mod1);
                p ++;
            }
            return resp % mod1;
        };
        vi vec;
        for(auto xd : adj[u]) if(xd != pp) vec.pb(data[xd].ta);
        if(!data[u].is_cut) ans += calcuate(vec);
        for(auto xd : adj[u]) if(xd != pp) dfs(xd, u, x);
    }
}pqTree;

int main() {
    // https://www.codechef.com/submit/GOODSEGS?tab=statement
    // doing dp on dfs after pqTree 
    fastio;
    ll n, k; cin >> n >> k;
    vi ar(n);
    for(auto &xd : ar) cin >> xd, xd --;
    pqTree.build(ar);
    assert (sz(pqTree.nodes) == 1);
    pqTree.dfs(pqTree.nodes.back().node, -1, k);
    cout << (pqTree.ans % mod1) << "\n";
    return 0;
}